# -----------------------------------------------------------------------------
# bot.py - نسخة احترافية (v5.3 - مرنة ومستقرة)
# -----------------------------------------------------------------------------
...
@app.route('/')
def health_check():
    return "Falcon Bot Service (v5.3 - Flexible & Stable) is Running!", 200
def run_server():
...
MIN_CONFIDENCE_BUY = 60  # عتبة مرنة
MIN_CONFIDENCE_SELL = 60 # عتبة مرنة
...
def save_watchlist(coins):
    with open(WATCHLIST_FILE, "w") as f: json.dump(coins, f)
...
def analyze_symbol(client, symbol):
    try:
        ...
        last = df.iloc[-1]
        confidence_buy, confidence_sell = 0, 0
        ...
        if confidence_buy >= MIN_CONFIDENCE_BUY: return 'BUY', last['close'], confidence_buy
        if confidence_sell >= MIN_CONFIDENCE_SELL: return 'SELL', last['close'], confidence_sell
    except Exception as e: logger.error(f"خطأ أثناء تحليل {symbol}: {e}")
    return 'HOLD', None, 0
...
async def scan_market(context):
    ...
    logger.info(f"--- بدء جولة الفحص (v5.3). العملات تحت المراقبة: {watchlist} ---")
...
def run_bot():
    ...
    if not all([TELEGRAM_TOKEN, TELEGRAM_CHAT_ID, BINANCE_API_KEY, BINANCE_SECRET_KEY]):
        logger.critical("!!! فشل: متغيرات البيئة غير كاملة. !!!"); return
    try:
        binance_client = Client(BINANCE_API_KEY, BINANCE_SECRET_KEY); binance_client.ping()
    except Exception as e: logger.critical(f"فشل الاتصال ببينانس: {e}"); return
    application = Application.builder().token(TELEGRAM_TOKEN).build()
    application.add_handler(CommandHandler("start", start))
    application.add_handler(CommandHandler("status", status))
    job_data = {'binance_client': binance_client, 'chat_id': TELEGRAM_CHAT_ID}
    application.job_queue.run_repeating(scan_market, interval=SCAN_INTERVAL_SECONDS, first=10, data=job_data)
