# -----------------------------------------------------------------------------
# bot.py - نسخة احترافية (v5.1 - إصلاح خطأ Pandas)
# -----------------------------------------------------------------------------
...
@app.route('/')
def health_check():
    return "Falcon Bot Service (Pro v5.1) is Running!", 200
def run_server():
...
async def scan_market(context):
    ...
    for symbol in list(bought_coins):
        ...
        if status == 'SELL':
            ...
            logger.info(f"💰 تم إرسال إشارة بيع للعملة: {symbol}")
    ...
    for symbol in symbols_to_scan:
        ...
        if status == 'BUY':
            ...
            save_watchlist(bought_coins)
            logger.info(f"🎯 تم إرسال إشارة شراء للعملة: {symbol}")
        await asyncio.sleep(0.5)
...
async def start(update, context):
    ...
    await update.message.reply_html(f"أهلاً {user.mention_html()}!\n\nأنا **بوت الصقر** (Pro v5.1) وجاهز للعمل.")

async def status(update, context):
    if bought_coins:
        coins_list = "\n".join(f"`{coin}`" for coin in bought_coins)
